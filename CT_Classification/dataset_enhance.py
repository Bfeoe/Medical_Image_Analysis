...
# 伪影纹理叠加
def overlay_texture(img, texture_img=None, alpha=0.5):
    # 如果没有提供texture_img，则生成随机纹理
    if texture_img is None:
        texture_img = np.random.randint(0, 50, img.shape, dtype=np.uint8)
    texture_img = cv2.resize(texture_img, (img.shape[1], img.shape[0]))
    img = cv2.addWeighted(img, 1 - alpha, texture_img, alpha, 0)
    return img
...
def color_jitter(img, brightness=0.2, contrast=0.2, saturation=0.2):
    img = img * (1.0 + np.random.uniform(-brightness, brightness))
    img_mean = np.mean(img, axis=(0, 1), keepdims=True)
    img = img_mean + (img - img_mean) * (1.0 + np.random.uniform(-contrast, contrast))
    hsv = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_RGB2HSV)
    hsv[..., 1] = hsv[..., 1] * (1.0 + np.random.uniform(-saturation, saturation))
...
# -------------------------------#
# 增强数据生成函数
def augment_image(img, num_variations):
    augmented_images = []
    for _ in range(num_variations):
        img_aug = img.copy()
        img_aug = random_flip_rotate(img_aug)
        img_aug = add_noise(img_aug, noise_type=np.random.choice(['gaussian', 'salt_pepper']))
        # img_aug = overlay_texture(img_aug, alpha=np.random.uniform(0.2, 0.5))
...
# 读取数据并划分数据集
def data_load(path, test_path, size, is_train, is_sampling='no_sampler', augmentations=20):
    dicomlist = []  # 图像地址
    labels = []  # 图像标签
    train_img = []  # 训练集图像
    train_label = []  # 训练集标签
    ...
    # 读取图像矩阵并进行预处理和增强
    images = []
    for idx, dcm in enumerate(dicomlist):
        img = pydicom.read_file(dcm).pixel_array
        img = data_preprocess_enhanced(img, size)
        images.append(img)

        # 如果是训练集且标签为'1'，进行数据增强
        if is_train and labels[idx] == '1':
            augmented_imgs = augment_image(img, augmentations)
            images.extend(augmented_imgs)
            labels = np.append(labels, ['1'] * augmentations)  # 扩展标签为'1'的数据
